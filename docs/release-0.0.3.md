...
The [versioned HTML guide](site/0.0.3/index.html) teaches the 0.0.3 language and
...
older mixed-feature examples are conceptual designs outside this version.
...
The last commands report `gloinc 0.0.3 (LLVM/MLIR 21.1.6)` and `42`.
...
`gloinc-0.0.3-macos-arm64.tar.gz` and a SHA-256 checksum. The archive contains:

| Path below the archive root | Contents |
| --- | --- |
| `bin/gloinc` | Compiler and in-process JIT client |
| `share/gloinc/scripts/install-llvm.sh` | Installer for the exact external LLVM/MLIR toolchain needed to run `gloinc` |
| `share/doc/gloinc/docs/site/0.0.3/` | Versioned HTML language and usage guide |
...
| `share/gloinc/core-fixtures/` | All 152 source acceptance fixtures and their matrix |

LLVM/MLIR and their Homebrew dependencies are external and are not redistributed
in this package. **Install LLVM/MLIR 21.1.6 before running even `gloinc --version`.**
The compiler links its shared libraries and uses its tools to build native output.
The installed executable uses the library installation selected
...
```sh
cd build
LC_ALL=C shasum -a 256 -c gloinc-0.0.3-macos-arm64.tar.gz.sha256
tar -xzf gloinc-0.0.3-macos-arm64.tar.gz
bash ./gloinc-0.0.3-macos-arm64/share/gloinc/scripts/install-llvm.sh
./gloinc-0.0.3-macos-arm64/bin/gloinc \
  --jit \
  ./gloinc-0.0.3-macos-arm64/share/gloinc/examples/core_counter.gloin
```
