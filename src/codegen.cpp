...
CodeGen::CodeGen(mlir::MLIRContext &ctx) : context(ctx), builder(&ctx) {
    ...
    if (mlir::succeeded(stringType.setBody({u8PtrType, lenType}, /*isPacked=*/false))) {
        ...
        struct_field_types["String"]["len"] = 1;
    }
    ...
    if (type_name.find("be_") == 0 || type_name.find("le_") == 0) {
        ...
        // Parse the base integer type
        mlir::Type _local_base_mlir_type;
        if (base_type == "i8")
            base_mlir_type = builder.getI8Type();
        else if (base_type == "i16")
            base_mlir_type = builder.getI16Type();
        else if (base_type == "i32")
            base_mlir_type = builder.getI32Type();
        else if (base_type == "i64")
            base_mlir_type = builder.getI64Type();
        else if (base_type == "i128")
            base_mlir_type = builder.getI128Type();
        else if (base_type == "u8")
            base_mlir_type = builder.getI8Type();
        else if (base_type == "u16")
            base_mlir_type = builder.getI16Type();
        else if (base_type == "u32")
            base_mlir_type = builder.getI32Type();
        else if (base_type == "u64")
            base_mlir_type = builder.getI64Type();
        else if (base_type == "u128")
            base_mlir_type = builder.getI128Type();
        else {
            // Unknown base type, fall back to i32
            base_mlir_type = builder.getI32Type();
        }

        // Create endianness-aware type
        static std::map<std::string, bool> local_endian_tracker;
    }
}
}
...
void CodeGen::emit_deferred() {
    ...
    while (ptr) {
        ...
        for (auto it = ptr->deferred.rbegin(); it != ptr->deferred.rend(); ++it) {
            // Generate MLIR DeferOp for each deferred statement
            if (auto *defer_stmt = dynamic_cast<const DeferStatement *>(*it)) {
                // Get the call expression from the defer statement
                if (defer_stmt->call) {
                    auto call_result = gen_expression(defer_stmt->call.get());

                    // Create DeferOp that wraps the call
                    auto defer_op =
                        builder.create<gloin::DeferOp>(builder.getUnknownLoc(), call_result);

                    // Add to current block (will be executed when scope exits)
                    builder.create<gloin::YieldOp>(builder.getUnknownLoc(), defer_op);
                }
            }
        }
        ptr = ptr->parent;
    ...
    if (auto *int_lit = dynamic_cast<const IntegerLiteral *>(expr)) {
        ...
        for (size_t i = 0; i < values.size(); ++i) {
            current = builder.create<mlir::LLVM::InsertValueOp>(
                builder.getUnknownLoc(), current, values[i], llvm::ArrayRef<int64_t>{(int64_t)i});
        }
        ...
        if (auto structType = llvm::dyn_cast<mlir::LLVM::LLVMStructType>(baseType)) {
            ...
            if (struct_field_offsets.count(structName) &&
                struct_field_offsets[structName].count(ident->value)) {
                ...
                auto bitcast = builder.create<mlir::LLVM::BitcastOp>(builder.getUnknownLoc(),
                                                                     intPtrType, baseAddr);

                auto val =
                    builder.create<mlir::LLVM::LoadOp>(builder.getUnknownLoc(), i32Type, bitcast);
                ...
                if (isBigEndian) {
                    ...
                    auto shiftAmt = builder.create<mlir::LLVM::ConstantOp>(
                        builder.getUnknownLoc(), i32Type, builder.getI32IntegerAttr(actual_shift));
                    shifted =
                        builder.create<mlir::LLVM::LShrOp>(builder.getUnknownLoc(), val, shiftAmt);
                } else {
                    // Little Endian: Offset is from LSB
                    auto shiftAmt = builder.create<mlir::LLVM::ConstantOp>(
                        builder.getUnknownLoc(), i32Type, builder.getI32IntegerAttr(offset));
                    shifted =
                        builder.create<mlir::LLVM::LShrOp>(builder.getUnknownLoc(), val, shiftAmt);
                }
                ...
                auto mask = builder.create<mlir::LLVM::ConstantOp>(
                    builder.getUnknownLoc(), i32Type, builder.getI32IntegerAttr(maskVal));
                auto masked =
                    builder.create<mlir::LLVM::AndOp>(builder.getUnknownLoc(), shifted, mask);
        ...
        if (auto *call = dynamic_cast<const CallExpression *>(spawn->call.get())) {
            ...
            auto spawnOp = builder.create<gloin::SpawnOp>(
                builder.getUnknownLoc(), spawnType, mlir::SymbolRefAttr::get(&context, funcName),
                operands);
        ...
        if (is_deferred) {
            ...
            auto asyncCall = builder.create<gloin::AsyncCallOp>(
                builder.getUnknownLoc(), deferredType, mlir::SymbolRefAttr::get(&context, funcName),
                operands);
        ...
        // - ScopeResolution? (Arena::new) -> Parser might handle this as Identifier("Arena::new")
        // or ScopeAccess
        ...
        if (auto *member = dynamic_cast<const MemberAccessExpression *>(call->function.get())) {
            ...
            if (method_name == "alloc") {
                ...
                auto sizeVal = builder.create<mlir::LLVM::ConstantOp>(
                    builder.getUnknownLoc(), builder.getI64Type(), builder.getI64IntegerAttr(size));
                ...
                auto allocFunc =
                    theModule.lookupSymbol<mlir::LLVM::LLVMFuncOp>("__gloin_arena_alloc");
                return builder
                    .create<mlir::LLVM::CallOp>(builder.getUnknownLoc(), allocFunc,
                                                mlir::ValueRange{arenaPtr, sizeVal})
                    .getResult();
                ...
                auto freeFunc =
                    theModule.lookupSymbol<mlir::LLVM::LLVMFuncOp>("__gloin_arena_free");
                builder.create<mlir::LLVM::CallOp>(builder.getUnknownLoc(), freeFunc,
                                                   mlir::ValueRange{arenaPtr});
                return nullptr;
            }
            ...
            auto callOp = builder.create<mlir::func::CallOp>(
                builder.getUnknownLoc(), mangledName, mlir::TypeRange{builder.getI32Type()}, args);
            ...
            if (funcName == "Arena::new") {
                ...
                auto undef =
                    builder.create<mlir::LLVM::UndefOp>(builder.getUnknownLoc(), arenaType);

                // malloc(4096)
                auto size = builder.create<mlir::LLVM::ConstantOp>(
                    builder.getUnknownLoc(), builder.getI64Type(), builder.getI64IntegerAttr(4096));
                auto mallocFunc = theModule.lookupSymbol<mlir::LLVM::LLVMFuncOp>("malloc");
                auto block = builder
                                 .create<mlir::LLVM::CallOp>(builder.getUnknownLoc(), mallocFunc,
                ...
                // ptr = block + 8
                auto i64Type = builder.getI64Type();
                auto eight = builder.create<mlir::LLVM::ConstantOp>(
                    builder.getUnknownLoc(), i64Type, builder.getI64IntegerAttr(8));
                auto blockInt =
                    builder.create<mlir::LLVM::PtrToIntOp>(builder.getUnknownLoc(), i64Type, block);
                auto ptrInt =
                    builder.create<mlir::LLVM::AddOp>(builder.getUnknownLoc(), blockInt, eight);
                auto ptr = builder.create<mlir::LLVM::IntToPtrOp>(builder.getUnknownLoc(), ptrType,
                                                                  ptrInt.getResult());

                // end = block + 4096
                auto endInt =
                    builder.create<mlir::LLVM::AddOp>(builder.getUnknownLoc(), blockInt, size);
                auto end = builder.create<mlir::LLVM::IntToPtrOp>(builder.getUnknownLoc(), ptrType,
                                                                  endInt.getResult());

                auto tmp1 = builder.create<mlir::LLVM::InsertValueOp>(
                    builder.getUnknownLoc(), undef, ptr, llvm::ArrayRef<int64_t>{0});
                auto tmp2 = builder.create<mlir::LLVM::InsertValueOp>(
                    builder.getUnknownLoc(), tmp1, end, llvm::ArrayRef<int64_t>{1});
                auto res = builder.create<mlir::LLVM::InsertValueOp>(
                    builder.getUnknownLoc(), tmp2, block, llvm::ArrayRef<int64_t>{2});
            ...
            auto callOp = builder.create<mlir::func::CallOp>(
                builder.getUnknownLoc(), funcName, mlir::TypeRange{builder.getI32Type()}, args);
            return callOp.getResult(0);
        }
        ...
        return builder.create<gloin::AwaitOp>(builder.getUnknownLoc(), resType, operand)
            .getResult();
...
void CodeGen::gen_function(const std::string &name, const std::vector<std::string> &args,
...
mlir::Type CodeGen::get_expression_type(const Expression *expr) {
    ...
    if (dynamic_cast<const StringLiteral *>(expr))
        return type_table["String"];

    if (auto *array_lit = dynamic_cast<const ArrayLiteral *>(expr)) {
        if (array_lit->elements.empty())
            return builder.getNoneType(); // Unknown
...
void CodeGen::handle_import(const std::string &import_path) {
    ...
    if (import_path.find("@std/") == 0) {
        // Standard library import - create stub functions
        handle_std_import(import_path.substr(5)); // Remove "@std/" prefix
    } else if (import_path.find("./") == 0 || import_path.find("../") == 0) {
        // Relative file import - would load from file system
        // For now, just print message
        std::cout << "Importing local module: " << import_path << std::endl;
    } else if (import_path.find("#") == 0) {
        // Package manager import
        std::cout << "Importing external package: " << import_path << std::endl;
    }
}
...
void CodeGen::handle_std_import(const std::string &module_name) {
    ...
    if (module_name == "io") {
        // Create print function stub
        auto voidType = builder.getNoneType();
        auto stringType = type_table["string"];
        auto funcType = builder.getFunctionType({stringType}, voidType);
        ...
        std::cout << "Created std::io::print function" << std::endl;
    } else if (module_name == "math") {
        // Create math functions
        std::cout << "Created std::math module" << std::endl;
    }
}
